              ...
              <div className="card">
                <div className="card-body p-4">
                  ...
                  <div className="p-2 mt-4">
                    {message && <div className="alert alert-success text-center mb-4 flash" role="alert">{message}</div>}
                    {error && <div className="alert alert-danger text-center mb-4 flash" role="alert">{error}</div>}
                    <form onSubmit={handleSubmit}>
                      <div className="mb-3">
                        <label className="form-label" htmlFor="email">Email</label>
                        <input type="text" className="form-control" id="email" name="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="Enter Email" />
                      </div>
                      <div className="mb-3">
                        <label className="form-label" htmlFor="userpassword">Contraseña</label>
                        <input type="password" className="form-control" id="userpassword" name="password" value={password} onChange={(e) => setPassword(e.target.value)} placeholder="Enter password" />
                      ...
                      <div className="form-check">
                        ...
                        <label className="form-check-label" htmlFor="auth-remember-check">Acepto Términos y Condiciones</label>
                      </div>
                      <div className="mt-3 text-end">
                        <Link to="/home">
                          <button className="btn btn-primary w-sm waves-effect waves-light" type="submit">Iniciar Sesión</button>
                        </Link>
                      </div>
                      ...
                      <div className="mt-4 text-center">
                        <p className="mb-0">¿No tienes una cuenta? <Link to="/Register" className="fw-medium text-primary">Regístrate ahora</Link></p>
                      </div>
                    </form>
                  </div>
                </div>
